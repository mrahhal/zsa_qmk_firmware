...
There are 4 functions for enabling and disabling the LEDs on the side of the boards. The functions are `STATUS_LED_1(bool)` through `STATUS_LED_4(bool)`, with the first LED being the left most LED on the left hand, and the fourth LED being the right most LED on the right side. 
